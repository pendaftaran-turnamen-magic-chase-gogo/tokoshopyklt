...
export function generateDynamicQris(staticQris: string, amount: number): string {
  ...
  // We stop before the last 4 chars (CRC value) 
  // But actually, we should parse until we hit tag 63
  
  // Let's try to parse the whole string first, ignoring the last CRC value for now
  // or just parse until end and overwrite 63.
  ...
  try {
    while (i < staticQris.length) {
      ...
      if (tag === '63') {
        // We found the CRC tag. We can stop or read it.
