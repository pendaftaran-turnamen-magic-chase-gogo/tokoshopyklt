...
import { generateDynamicQris, DEFAULT_STATIC_QRIS } from '../lib/qris';
  ...
  const getGeolocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => { 
        ...
        if (err.code === 1) {
            msg = 'Izin lokasi ditolak. Mohon izinkan akses lokasi di pengaturan browser Anda, lalu coba lagi.';
            // Prompt user to check settings
            alert(msg);
        } else if (err.code === 2) {
            msg = 'Posisi tidak tersedia. Pastikan GPS/Lokasi di perangkat Anda aktif.';
            alert(msg);
        } else if (err.code === 3) {
          ...
          <div className="bg-white w-full max-w-sm rounded-[48px] overflow-hidden shadow-2xl relative max-h-[90vh] overflow-y-auto">
            {activeTx.status === 'pending' ? (
                <div className="p-8 text-center">
                    ...
                    {activeTx.type === 'qris' ? (
                        <>
                            <div className="bg-white p-3 border-4 border-rose-600 rounded-[32px] inline-block mb-4 shadow-xl relative group min-h-[250px] flex items-center justify-center overflow-hidden">
                                {activeTx.type === 'qris' ? (
                                  <div className="flex flex-col items-center">
                                    <QRCodeSVG 
                                      value={generateDynamicQris(DEFAULT_STATIC_QRIS, activeTx.total)} 
                                      size={200} 
                                      level="M" 
                                    />
